...
class HomeController extends Controller
{
    ...
    {
        // if the user type is seller then page title is seller dashboard, if buyer then buyer dashboard
        if(auth()->user()->type == 'seller'){
            $data['page_title'] = "Seller Dashboard";
            $data['total_trades_this_month'] = Trade::where('seller_id', auth()->user()->id)->whereMonth('created_at', Carbon::now()->month)->count();
            // total trades last month
            $data['total_trades_last_month'] = Trade::where('seller_id', auth()->user()->id)->whereMonth('created_at', Carbon::now()->subMonth()->month)->count();
            // total trades this month and last month with status closed
            $data['total_trades_this_month_closed'] = Trade::where('seller_id', auth()->user()->id)->whereMonth('created_at', Carbon::now()->month)->where('status', 'closed')->count();
            $data['total_trades_last_month_closed'] = Trade::where('seller_id', auth()->user()->id)->whereMonth('created_at', Carbon::now()->subMonth()->month)->where('status', 'closed')->count();
            // total closed trades
            $data['total_closed_trades'] = Trade::where('seller_id', auth()->user()->id)->where('status', 'closed')->count();
        }else{
            $data['page_title'] = "Buyer Dashboard";
        }
        $user = Auth::user();

        // id user type is seller, then get all Trade with seller_id = user_id into data array
        if ($user->type == 'seller') {
            $data['trades'] = Trade::where('seller_id',$user->id)->latest()->take(10)->get();
        } else {
            $data['trades'] = Trade::where('buyer_id',$user->id)->latest()->take(10)->get();
        }
